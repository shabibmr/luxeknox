...
- [x] API-000 **Reconcile the documented-but-unimplemented backlog.** — **Verified:** all backlog routes reconciled: `POST /auth/password/change` + `forgot` + `reset` in `auth.controller.ts`; `roles` + `permissions` CRUD in `role.controller.ts`; `GET /audit-logs` in `audit-log.controller.ts`. Unimplemented routes (`health-conditions`, `medical-histories`, `PUT /settings`) are explicitly marked `x-status: deferred`. All 152 MVP routes annotated with `x-parity: required` and pass `check-openapi-parity.ts` live dump assertions with 0 allowlist entries.
